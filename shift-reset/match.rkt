...
;; (Nov 2012)
;; Ron Garcia (rxg@cs.ubc.ca) ported enough of it to Racket v5.3 so that a basic use of match with CATA
;; vars works.  
...
(define-syntax ->
  (lambda (x)
    (raise-syntax-error #f "misplaced aux keyword" x)))
  ...
  (lambda (x)
    (syntax-case x ()
      ((_ ((Cvar Cdepth MyCata Cformal ...) ...) B)
       (with-syntax (((CF ...) (apply append (syntax->list #'((Cformal ...) ...)))))
         #'(let-syntax
               ((CF
                  (lambda (x)
                    (syntax-case x ()
      ...
      (syntax-case syn (unquote)
        ...
        (((unquote . stuff) Dots)
         ...
         (syntax-case syn (unquote ->)
           ...
           ((expr Dots) (raise-syntax-error #'expr "match-pattern unquote syntax"))))
        ((Pat Dots)
         (ellipsis? #'Dots)
          ...
          ((Exp Dots . Rest)
           ...
           (with-values (destruct Orig #'Exp depth)
             (syntax-lambda (ExpBuilder (ExpVar ...) (ExpExp ...))
               (if (null? #'(ExpVar ...))
                   (raise-syntax-error Orig "Bad ellipsis")
                   (with-values (destruct Orig #'Rest depth)
                     (syntax-lambda (RestBuilder RestVars RestExps)
                       (with-syntax ((TailExp
                                       (if (null? #'RestVars)
